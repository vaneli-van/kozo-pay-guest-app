...
// Google Review handoff: returns the restaurant's write-a-review deep link. Shown only after positive
// private feedback (the client gates on sentiment). No PII leaves the app.
  ...
  server: { handlers: {
    ...
    POST: async ({ request }) => {
      try {
        ...
        const placeId = restaurant?.google_place_id ?? null
