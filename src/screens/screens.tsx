...
export function Connect({ dispatch }: any) { return <Center eyebrow="A BETTER WAY TO DINE" title={'Making the<br /><em>table</em> feel closer.'} copy="Connecting to your table…"><div className="loader" /><button className="text-link" onClick={() => dispatch(go('welcome'))}>Skip connection</button></Center> }

export function Welcome({ s, dispatch }: any) { return <section className="welcome"><div className="hero-image" /><div className="welcome-copy"><p className="eyebrow">WELCOME TO {(s?.restaurantName || 'the restaurant').toUpperCase()}</p><h1>Good food.<br /><em>Good company.</em></h1><p className="muted">{s.tableLabel ? `You are at table ${s.tableLabel}.` : 'You are at your table.'} Explore the menu, call your waiter, or settle your bill when you are ready.</p><Action onClick={() => dispatch(go(s.hasOrder ? 'bill' : 'empty'))}>{s.hasOrder ? 'View your live bill' : 'Explore the menu'}</Action></div></section> }
...
export function WaiterNotified({ dispatch }: any) { return <Center eyebrow="REQUEST SENT" title={'On the<br /><em>way.</em>'} copy="Your waiter has been notified. No need to wave — we will come to you."><div className="notice-card"><Clock3 /><span>Usually within 2–3 minutes</span></div><Action onClick={() => dispatch(go('waiting-bill'))}>Wait for my bill</Action><button className="text-link" onClick={() => dispatch(go('menu'))}>Back to menu</button></Center> }

export function WaitingBill({ dispatch }: any) { return <Center eyebrow="BILL REQUESTED" title={'We are<br /><em>on it.</em>'} copy="Your bill will appear here once your waiter closes the table."><div className="loader" /><button className="text-link" onClick={() => dispatch(go('welcome'))}>Back to your table</button></Center> }
...
export function Success({ s, dispatch }: any) {
  ...
  return <Center eyebrow="PAYMENT COMPLETE" title={'You&apos;re all<br /><em>settled.</em>'} copy={`Thanks for dining at ${s?.restaurantName || 'us'}. Your receipt is ready whenever you are.`} icon="✓">
...
export function Rewards({ s, dispatch }: any) { return <Center eyebrow="REWARDS SAVED" title={'See you<br /><em>again.</em>'} copy={`Your receipt is saved and 120 ${s?.restaurantName || ''} points have been added.`} icon="★"><Action onClick={() => dispatch(go('feedback'))}>Share feedback</Action></Center> }

export function GuestReceipt({ s, dispatch }: any) { return <section><p className="eyebrow">GUEST RECEIPT · {s?.receiptNumber ?? '#2841'}</p><h1>All <em>done.</em></h1><div className="receipt-card"><div className="receipt-head"><span>{s?.restaurantName || ''}</span><b>PAID</b></div><p>Your receipt is available for this session.</p><div className="grand-total"><span>Total paid</span><b>{money((s?.totalPaidPesewas ?? 38115) / 100)}</b></div></div><Action onClick={() => dispatch(go('feedback'))}>Continue</Action></section> }
...
export function ReviewHandoff({ s, dispatch }: any) { return <Center eyebrow="THANK YOU" title={'Would you tell<br /><em>Google too?</em>'} copy={`Thanks for your feedback. If you have a moment, a quick Google review helps other diners find ${s?.restaurantName || 'us'}. It opens in a new tab.`} icon="★"><Action onClick={() => { try { if (s?.reviewUrl) window.open(s.reviewUrl, '_blank', 'noopener') } catch {} dispatch(go('complete')) }}>Leave a Google review</Action><button className="text-link" onClick={() => dispatch(go('complete'))}>No thanks</button></Center> }

export function Complete({ dispatch }: any) { return <Center eyebrow="THANK YOU" title={'Until the<br /><em>next one.</em>'} copy="Your feedback helps us make every table feel closer."><button className="demo-control" onClick={() => dispatch({ type: 'reset' })}><RotateCcw />Start again</button></Center> }
