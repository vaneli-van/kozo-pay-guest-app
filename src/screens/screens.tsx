...
export function Welcome({ s, dispatch }: any) {
  ...
  return <section className="welcome"><div className="hero-image" {...(s?.heroUrl ? { style: { backgroundImage: `url(${s.heroUrl})` } } : {})} /><div className="welcome-copy"><p className="eyebrow">WELCOME TO {(s?.restaurantName || 'the restaurant').toUpperCase()}</p><h1><em>{s?.taglineBottom || s?.restaurantName || 'Welcome'}</em></h1><div className="welcome-info">{oc && <span className="welcome-status" style={{ color: oc.open ? '#1c7c3a' : '#8a857c' }}>{oc.open ? '\u25CF Open now' : '\u25CF Closed now'}</span>}{hrs && <p>{hrs}</p>}{dig.info && <p>{dig.info}</p>}{dig.phone && <p>Tel: {dig.phone}</p>}{dig.link_url && <p><a href={dig.link_url} target="_blank" rel="noopener noreferrer">{dig.link_text || 'Visit website'}</a></p>}</div><p className="muted">{s.tableLabel ? `You're at table ${s.tableLabel}. Browse the menu and settle your bill whenever you're ready.` : "Browse the menu and settle your bill whenever you're ready."}</p><Action onClick={() => dispatch(go('menu'))}>View the menu</Action></div></section>
}
...
export function WaitingBill({ s, dispatch }: any) { const isOrder = s?.mode === 'order'; return <Center logoUrl={s?.logoUrl} alt={s?.restaurantName} eyebrow={isOrder ? 'ALMOST THERE' : 'BILL REQUESTED'} title={isOrder ? 'Waiting for<br /><em>your order.</em>' : 'We are<br /><em>on it.</em>'} copy={isOrder ? 'Ask the cashier to send your order to Klown. It appears here the moment they do.' : 'Your bill will appear here once your waiter closes the table.'}><div className="loader" />{isOrder ? null : <button className="text-link" onClick={() => dispatch(go('welcome'))}>Back to your table</button>}</Center> }

export function Bill({ s, dispatch, ready = false }: any) { const b = s?.bill; const items = b?.items ?? []; if (s?.mode === 'order') { return <section><Back dispatch={dispatch} to={ready ? 'bill-ready' : 'waiting-bill'} /><p className="eyebrow">YOUR ORDER · {s?.tableLabel ?? ''}</p><h1>Your order,<br /><em>ready to pay.</em></h1><div className="bill-card"><div className="status"><span className="pulse" />From {s?.restaurantName || ''} counter <Clock3 /></div>{items.map((i: any, n: number) => <BillRow key={`${i.name}-${n}`} name={i.name} qty={String(i.qty)} price={i.lineTotalPesewas / 100} />)}<div className="grand-total"><span>Total</span><b>{pes(b?.totalPesewas)}</b></div></div><Action onClick={() => dispatch(go('pay'))}>Pay {pes(b?.totalPesewas)}</Action></section> } const tb = taxBreakdown(b?.subtotalPesewas ?? b?.totalPesewas ?? 0); return <section><Back dispatch={dispatch} to={ready ? 'bill-ready' : 'welcome'} /><p className="eyebrow">TABLE {s?.tableLabel ?? ''} · LIVE BILL</p><h1>Your table,<br /><em>so far.</em></h1><div className="bill-card"><div className="status"><span className="pulse" />{ready ? `Bill ready from ${s?.restaurantName || ''} POS` : `Live from ${s?.restaurantName || ''} POS`} <Clock3 /></div><p>{s?.people ?? 2} guests · Started 7:42 PM</p>{items.map((i: any, n: number) => <BillRow key={`${i.name}-${n}`} name={i.name} qty={String(i.qty)} price={i.lineTotalPesewas / 100} />)}<div className="tax-lines"><div className="tax-row"><span>Net (excl. tax)</span><span>{pes(tb.net)}</span></div><div className="tax-row"><span>NHIL 2.5%</span><span>{pes(tb.nhil)}</span></div><div className="tax-row"><span>GETFund 2.5%</span><span>{pes(tb.getfund)}</span></div><div className="tax-row"><span>VAT 15%</span><span>{pes(tb.vat)}</span></div><div className="tax-row"><span>Tourism Levy 1%</span><span>{pes(tb.tourism)}</span></div></div><div className="bill-total"><span>Subtotal</span><b>{pes(b?.subtotalPesewas)}</b></div><div className="bill-total"><span>Service charge</span><span>{pes(b?.serviceChargePesewas)}</span></div><div className="grand-total"><span>Total so far</span><b>{pes(b?.totalPesewas)}</b></div></div><button className="waiter-link" onClick={() => dispatch(go('bill-issue'))}><Info />Report a problem with the bill</button><Action onClick={() => dispatch(go('pay'))}>Settle the bill</Action></section> }

export function FullCheck({ s, dispatch }: any) { const b = s?.bill; const tb = taxBreakdown(b?.subtotalPesewas ?? b?.totalPesewas ?? 0); return <section><Back dispatch={dispatch} to="bill" /><p className="eyebrow">FULL CHECK · TABLE 07</p><h1>Everything<br /><em>looks good.</em></h1><div className="receipt-card"><BillRow name={`Dinner for ${s?.people ?? 2}`} qty="" price={(b?.subtotalPesewas ?? 0) / 100} /><BillRow name="Service charge" qty="" price={(b?.serviceChargePesewas ?? 0) / 100} /><div className="tax-lines"><div className="tax-row"><span>Net (excl. tax)</span><span>{pes(tb.net)}</span></div><div className="tax-row"><span>NHIL 2.5%</span><span>{pes(tb.nhil)}</span></div><div className="tax-row"><span>GETFund 2.5%</span><span>{pes(tb.getfund)}</span></div><div className="tax-row"><span>VAT 15%</span><span>{pes(tb.vat)}</span></div><div className="tax-row"><span>Tourism Levy 1%</span><span>{pes(tb.tourism)}</span></div></div><div className="grand-total"><span>Total</span><b>{pes(b?.totalPesewas)}</b></div></div><Action onClick={() => dispatch(go('recommendation'))}>Continue</Action></section> }

export function Recommendation({ s, dispatch }: any) {
  const dig = s?.menu?.digital || {}
  const th = s?.menu?.theme || {}
  const plain = th?.layout?.price_style === 'plain'
  const fmtP = (p: number | null | undefined) => (p == null ? '' : plain ? (p / 100).toLocaleString('en-GH', { maximumFractionDigits: 2 }) : pes(p))
  const hasRec = !!(dig.rec_name && String(dig.rec_name).trim())
  if (hasRec) {
    return <section><Back dispatch={dispatch} to="full-check" /><p className="eyebrow">BEFORE YOU GO</p><h1>One last<br /><em>little thing?</em></h1><div className="recommend-card">{dig.rec_image_url ? <img src={dig.rec_image_url} alt={dig.rec_name} onError={imgFallback} /> : null}<div><p className="eyebrow">CHEF’S PICK</p><h2>{dig.rec_name}</h2>{dig.rec_note ? <p>{dig.rec_note}</p> : null}{dig.rec_price_pesewas != null ? <b>{fmtP(dig.rec_price_pesewas)}</b> : null}</div></div><Action onClick={() => dispatch(go('pay'))}>Settle the bill</Action></section>
  }
  if (s?.menu?.source === 'studio') {
    return <section><Back dispatch={dispatch} to="full-check" /><p className="eyebrow">BEFORE YOU GO</p><h1>Ready to<br /><em>settle up?</em></h1><p className="muted">Review your check and pay whenever you’re ready.</p><Action onClick={() => dispatch(go('pay'))}>Settle the bill</Action></section>
  }
  const recs = s?.menu?.recommendations ?? []; const rec = recs.find((r: any) => r.kind === 'dessert') ?? recs[0]; const it = rec ? (s?.menu?.items ?? []).find((i: any) => i.id === rec.item_id) : undefined
...
export function PaymentError({ s, dispatch }: any) {
  const isCard = s?.method === 'card'
  const reason = (s?.failureReason || '').toString().trim()
  const retryTo = isCard ? 'processing' : 'momo'
  return <section><Back dispatch={dispatch} to="method" /><p className="eyebrow">{isCard ? 'BANK CARD' : 'MOBILE MONEY'} · PAYMENT</p><h1>Payment<br /><em>didn&apos;t go through.</em></h1><p className="muted">{isCard ? 'Your card wasn\u2019t charged. This can happen if the card was declined, the details didn\u2019t match, or the payment page closed before it finished.' : 'No money left your account. Check your mobile-money balance and that you approved the prompt, then try again.'}</p>{reason && <div className="error"><X />{reason}</div>}<Action onClick={() => dispatch({ type: 'patch-go', value: { paymentRef: undefined, failureReason: undefined }, to: retryTo })}>Try again</Action><button className="text-link" onClick={() => dispatch({ type: 'patch-go', value: { paymentRef: undefined, failureReason: undefined }, to: 'method' })}>Choose another method</button></section>
}

export const map: Record<string, any> = { connect: Connect, welcome: Welcome, empty: Empty, menu: Menu, category: Category, dish: Dish, waiter: Waiter, 'waiter-notified': WaiterNotified, 'waiting-bill': WaitingBill, 'bill-ready': (p: any) => <Bill {...p} ready />, bill: Bill, 'bill-issue': BillIssue, 'full-check': FullCheck, recommendation: Recommendation, pay: Pay, split: Split, 'split-share': (p: any) => <SplitShare {...p} />, 'split-lobby': (p: any) => <SplitLobby {...p} />, 'split-items': (p: any) => <SplitItems {...p} />, tip: Tip, review: Review, method: Method, momo: Momo, otp: PaymentOtp, authorise: Authorise, processing: Processing, 'payment-error': (p: any) => <PaymentError {...p} />, success: Success, 'receipt-choice': ReceiptChoice, phone: Phone, 'otp-rewards': OtpRewards, name: Name, rewards: Rewards, 'guest-receipt': GuestReceipt, feedback: Feedback, 'review-handoff': ReviewHandoff, complete: Complete }

// Diner render for a published Menu Studio menu (themed). Display-only; bill/pay unchanged.
function StudioMenu({ s }: any) {
  ...
  if (!sections.length) {
    return (
      <section>
        <header className="page-header"><div><p className="eyebrow">{(s?.restaurantName || '').toUpperCase()}</p><h1>Menu<br /><em>coming soon.</em></h1></div></header>
      </section>
    )
  }
    ...
    <section className="studio-menu" style={{ background: t.colors.paper, color: t.colors.ink, fontFamily: t.fonts.body, margin: '-22px -20px 0', padding: '22px 20px', minHeight: 'calc(100dvh - 60px)' }}>
      <div style={{ position: 'fixed', top: 0, left: '50%', width: 'min(100%, 480px)', zIndex: 30, background: t.colors.paper, borderBottom: '1px solid rgba(0,0,0,0.08)', transform: scrolled ? 'translate(-50%, 0)' : 'translate(-50%, -110%)', transition: 'transform .22s ease', paddingTop: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px 8px' }}>
          ...
          {s?.tableLabel && <span style={{ fontSize: 10, letterSpacing: '.1em', color: t.colors.accent, fontWeight: 700 }}>TABLE {s.tableLabel}</span>}
        </div>
        {tabsEl}
      </div>
      ...
      {sections.map((section: any) => (
        <div className="menu-group" key={section.id} id={'sec-' + section.id} data-sid={section.id} style={{ scrollMarginTop: 96 }}>
